    ...
    @else
        <div class="border border-amber-100 rounded-sm divide-y divide-amber-100 bg-amber-50/30 max-h-[420px] overflow-y-auto thin-scrollbar">
            @foreach($rows as $row)
                @if(isset($row->csa_id))
                    <div
                        onClick="window.location.href='{{ route('readings.csas.show',$row->csa_id) }}'" 
                        
                        class="flex items-center gap-3 px-4 py-3 hover:bg-amber-50/60 transition cursor-pointer">
